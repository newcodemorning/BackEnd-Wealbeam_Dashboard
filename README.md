...
- Added multilingual support (Arabic and English) for PDF titles and descriptions
- PDFs now support `title: { en: '...', ar: '...' }` and `description: { en: '...', ar: '...' }`
- Language parameter in URL automatically returns content in requested language
- Created migration endpoint `/pdf/migrate` to update existing PDFs to new format
- Migration function sets both `ar` and `en` fields to existing value initially

#### Description

> **Multilingual PDF Support**  
> PDFs now support Arabic and English for titles and descriptions, similar to the blog system.
>
> - Use language prefix in URL: `/en/pdf/...` or `/ar/pdf/...`
> - PDFs are automatically returned in the requested language
> - Run migration once: `POST /pdf/migrate` (super-admin only)
>
> **API Examples:**
>
> - Create PDF: Send `title[en]`, `title[ar]`, `description[en]`, `description[ar]`
> - Get PDFs: `GET /en/pdf/` returns English content, `GET /ar/pdf/` returns Arabic content
